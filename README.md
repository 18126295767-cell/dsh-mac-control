...
外掛不啟動 HTTP 服務、不代理瀏覽器流量，也不儲存憑據；它只把已驗證要求交給 macOS `osascript`、`open` 與 `screencapture`，變更操作預設需要確認。官方路徑使用 [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) 的 `@deepseek-ai/dsh@0.1.0-rc.7`，以 `plugin --profile web add` 安裝審閱過的 commit archive。
